...
function FindMSPath([string] $DevEnvExe, [string] $TargetExeName)
{
  ...
  while ($null -eq $result) { # null must come first
    $parentDir = [System.IO.Directory]::GetParent($parentDir)
    if (Test-Path $parentDir) { # Ensure no endless loop
      Set-Location $parentDir
      $result = Get-ChildItem -Include $TargetExeName -Recurse | Where-Object {$_.FullName -notlike '*amd64*'}
    } else {
      break
...
if (TestProjectPaths -and TestExecutablePaths){ # else error written to console
	BuildDacpac
  GenerateDiffScript
  PopDiffScript
}