...
#This goes in the config file and tells git that there is a merge driver for merging .resource files
...
[merge "database-resource-script-merge-driver"]
    ...
    driver = database-resource-merge-driver.sh %O %A %B
'@
...
# This is shell script referenced by the entry in config
$MergeDriverEntryPointPath = './database-resource-merge-driver.sh'
$MergeDriverEntryPointContents = 'pwsh ResolveScriptResourceDifferences.ps1 $1 $2 $3'
# The entry point shell script hops over to powershell.
...
# Write out the entry point shell script
Set-Content -Path $MergeDriverEntryPointPath $MergeDriverEntryPointContents
